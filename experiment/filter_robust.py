...
from core.modules_statistics import HookModule
...
def screen(model, train_loader, num_classes, device, save_dir):
    hookModule = HookModule(model, layers, num_classes)
...
def screen_c(model, train_loader, num_classes, device, save_dir):
    hookModule = HookModule(model, layers, num_classes)
    with tqdm(total=len(train_loader), ncols=80)as pbar:
        for idx, samples in tqdm(enumerate(train_loader)):
            inputs, labels = samples
...
def main():
    ...
    if corruption == True:
        data_config = config.get_data_config(data_name + 'C')
        corruptions = data_config['corruptions']

        for idx, cname in enumerate(corruptions):
            print("**{}:".format(cname))
            data_loader = loader.load_data_C(data_name + 'C', cname)
            screen_c(model, data_loader, num_classes, device, save_dir)
